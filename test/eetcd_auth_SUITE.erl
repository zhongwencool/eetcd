...
user(_Config) ->
    {ok, #'Etcd.AuthEnableResponse'{}} = eetcd_auth:auth_enable(#'Etcd.AuthEnableRequest'{}),
    %% list
    {ok, #'Etcd.AuthRoleListResponse'{roles = []}}
        = eetcd_auth:user_list(#'Etcd.AuthRoleListRequest'{}),
    %% add
    {ok, #'Etcd.AuthRoleAddResponse'{}}
        = eetcd_auth:user_add(#'Etcd.AuthUserAddRequest'{name = <<"name1">>, password = <<"12345">>}),
    %% get
    {ok, #'Etcd.AuthUserGetResponse'{roles = [<<"name1">>]}}
        = eetcd_auth:user_get(#'Etcd.AuthUserGetRequest'{name = <<"name1">>}),
    %% change password
    ...
    {ok, #'Etcd.AuthUserGetResponse'{roles = [<<"name1">>]}}
        = eetcd_auth:user_get(#'Etcd.AuthUserGetRequest'{name = <<"name1">>}),
    %% delete
    {ok, #'Etcd.AuthUserDeleteResponse'{}}
        = eetcd_auth:user_delete(#'Etcd.AuthUserDeleteRequest'{name = <<"name1">>}),
    %% list
